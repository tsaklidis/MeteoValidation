...
for i in range(len(station['temps']) - len(meteo['temps'])):
    meteo['temps'].append(None)

# Add data to plot, multiple data can be added
# plt.plot(x, y1)
# plt.plot(x, y2)
plt.plot(station['dates'], meteo['temps'])
plt.plot(station['dates'], station['temps'])
